%Karol Wadolowski - Project #3: MIMO-OFDM (Part 3)
clear; clc; close all;

N = 3e3;                %Number of symbols to generate
SNRdB = 35;             %SNR in dB
...
for ii = 1:length(fD)
    ...
    for jj = 1:length(M)
        for kk = 1:length(R)
            %Generate Data
            [DataBits,data,H,dr] = genMIMO_OFDMdata(2,M(jj),R(kk),N,fD(ii));
            DR(jj,kk) = dr;
            
            %Send and Decode data various ways
            berSVD = precodingMIMO_OFDM(2,M(jj),R(kk),SNRdB,data,H,DataBits);
            [berMMSE,berZF] = mmse_zfMIMO_OFDM(2,M(jj),R(kk),SNRdB,data,H,...
                DataBits);
...
for ii = 1:length(fD)
    ...
    for jj = 1:length(targBER)
        ...
        fprintf('Target BER = %.0e, SNR = %.1f dB, %.1e OFDM Symbols used\n',...
            targBER(jj),SNRdB,N)
